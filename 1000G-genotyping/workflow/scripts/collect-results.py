...
def collect_concordances(folder, variant, outname, source, samples, regions):
	with open(outname + '_' + regions + '_' + variant + '.tsv', 'w') as outfile:
		outfile.write('\t'.join(['sample', 'weighted_genotype_concordance', 'typed_variants']) + '\n')
		for i, sample in enumerate(samples):
			filename = folder + source + '/' + sample + '/evaluation/concordance/pangenie/' + regions + '_' + variant + '/summary.txt'
			concordance, typed = parse_concordance(filename)
			outfile.write('\t'.join([sample, str(concordance), str(typed)]) + '\n')


def collect_precision_recall(folder, variant, outname, source, samples, regions):
	with open(outname + '_' + regions + '_' + variant + '.tsv', 'w') as outfile:
		outfile.write('\t'.join(['sample', 'precision', 'recall', 'fscore']) + '\n')
		for i, sample in enumerate(samples):
			filename = folder + source + '/' + sample + '/evaluation/precision-recall-typable/pangenie/' + regions + '_' + variant + '/summary.txt'
...
if __name__ == '__main__':
	...
	parser.add_argument('source', metavar='SOURCE', help='source')
	...
	if args.metric == 'concordance':
		collect_concordances(args.folder, args.variants, args.outfile, args.source, samples, args.regions)
	if args.metric == 'precision-recall-typable':
		collect_precision_recall(args.folder, args.variants, args.outfile, args.source, samples, args.regions)
